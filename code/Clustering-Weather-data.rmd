...
```{r}
stn_789_nov_avged_loc %>% filter(YEAR==2007) -> stn_7_nov_avged_loc
maxk<-12
set.seed(110)
mediods_7<-sample(1:count(stn_7_nov_avged_loc)$n,maxk)
kmn_model<-stn_7_nov_avged_loc %>% ungroup() %>% select(TEMP,DEWP,STP,WDSP) %>% pam(x=.,k = 2,stand = TRUE,metric = "euclidean")
...
while(k <= maxk){
  kmn_model<-stn_7_nov_avged_loc %>% ungroup() %>% select(TEMP,DEWP,STP,WDSP) %>% pam(x=.,k = k,stand = TRUE,metric = "euclidean",medoids = mediods_7[1:k])
  resultMatrix %>% add_row(Metric="euclidean",K=k,SSE=kmn_model$objective["swap"],YEAR=2007) -> resultMatrix
  k<-k+1
}
```
...
```{r}
stn_789_nov_avged_loc %>% filter(YEAR==2008) -> stn_8_nov_avged_loc
set.seed(110)
mediods_8<-sample(1:count(stn_8_nov_avged_loc)$n,maxk)
k<-2
while(k <= maxk){
  kmn_model<-stn_8_nov_avged_loc %>% ungroup() %>% select(TEMP,DEWP,STP,WDSP) %>% pam(x=.,k = k,stand = TRUE,metric = "euclidean",medoids = mediods_8[1:k])
  resultMatrix %>% add_row(Metric="euclidean",K=k,SSE=kmn_model$objective["swap"],YEAR=2008) -> resultMatrix
  k<-k+1
}
```
...
```{r}
stn_789_nov_avged_loc %>% filter(YEAR==2009) -> stn_9_nov_avged_loc
set.seed(110)
mediods_9<-sample(1:count(stn_9_nov_avged_loc)$n,maxk)
k<-2
while(k <= maxk){
  kmn_model<-stn_9_nov_avged_loc %>% ungroup() %>% select(TEMP,DEWP,STP,WDSP) %>% pam(x=.,k = k,stand = TRUE,metric = "euclidean",medoids = mediods_9[1:k])
  resultMatrix %>% add_row(Metric="euclidean",K=k,SSE=kmn_model$objective["swap"],YEAR=2009) -> resultMatrix
  k<-k+1
}
```
...
```
#Clustering year 2007 k=2 to 8 using Manhattan metric
```{r}
stn_789_nov_avged_loc %>% filter(YEAR==2007) -> stn_7_nov_avged_loc
maxk<-12
set.seed(110)
mediods_7<-sample(1:count(stn_7_nov_avged_loc)$n,maxk)
kmn_model<-stn_7_nov_avged_loc %>% ungroup() %>% select(TEMP,DEWP,STP,WDSP) %>% pam(x=.,k = 2,stand = TRUE,metric = "manhattan")
ggplot(stn_789_nov_avged_loc %>% filter(YEAR==2007) %>% select(Lon,Lat) %>% mutate(clustAllocated=as.factor(kmn_model$cluster)) , aes(Lon, Lat,colour=clustAllocated)) +
  geom_point(size = 1, show.legend = TRUE) +
  coord_quickmap()
k<-2
...
while(k <= maxk){
  kmn_model<-stn_7_nov_avged_loc %>% ungroup() %>% select(TEMP,DEWP,STP,WDSP) %>% pam(x=.,k = k,stand = TRUE,metric = "manhattan",medoids = mediods_7[1:k])
  resultMatrix %>% add_row(Metric="manhattan",K=k,SSE=kmn_model$objective["swap"],YEAR=2007) -> resultMatrix
  k<-k+1
}
```
...
k<-2
while(k <= maxk){
  kmn_model<-stn_8_nov_avged_loc %>% ungroup() %>% select(TEMP,DEWP,STP,WDSP) %>% pam(x=.,k = k,stand = TRUE,metric = "manhattan",medoids = mediods_8[1:k])
  resultMatrix %>% add_row(Metric="manhattan",K=k,SSE=kmn_model$objective["swap"],YEAR=2008) -> resultMatrix
  k<-k+1
}
```
...
k<-2
while(k <= maxk){
  kmn_model<-stn_9_nov_avged_loc %>% ungroup() %>% select(TEMP,DEWP,STP,WDSP) %>% pam(x=.,k = k,stand = TRUE,metric = "manhattan",medoids = mediods_9[1:k])
  resultMatrix %>% add_row(Metric="manhattan",K=k,SSE=kmn_model$objective["swap"],YEAR=2009) -> resultMatrix
  k<-k+1
}
```
