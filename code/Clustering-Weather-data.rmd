...
#Clustering year 2007 k=2
```{r}
set.seed(110)
kmn_model<-stn_789_nov_avged_loc %>% filter(YEAR==2007) %>% ungroup() %>% select(TEMP,DEWP,STP,WDSP) %>% clara(x=.,k = 2)
ggplot(stn_789_nov_avged_loc %>% filter(YEAR==2007) %>% select(Lon,Lat) %>% mutate(clustAllocated=kmn_model$cluster) , aes(Lon, Lat,colour=clustAllocated)) +
